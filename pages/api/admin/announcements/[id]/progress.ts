...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const progress = await getDeliveryProgress(ItemType.ANNOUNCEMENT, announcement.id)
    return res.status(200).json(progress)
  } catch (error: any) {
    return res.status(500).json({ error: error.message || 'Failed to fetch announcement progress' })
  }
}
